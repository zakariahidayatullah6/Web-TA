    ...
    elseif (!preg_match("/^[a-zA-Z 0-9]*$/", $up_name)) {
        ...
        if ( !mysqli_stmt_prepare($result, $sql)){
            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                $pwdCheck = password_verify($up_password, $row['login_pwd']);
                if ($pwdCheck == false) {
                    ...
                    if ($useremail == $up_email) {
                        $sql = "UPDATE login SET login_name=? WHERE login_email=?";
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            ...
                            $_SESSION['login-name'] = $up_name;
                            header("location: index.php?success=updated");
                            exit();
                        }
                    }
                    else{
                        $sql = "SELECT login_email FROM login WHERE login_email=?";  
                        $result = mysqli_stmt_init($conn);
                        if ( !mysqli_stmt_prepare($result, $sql)){
                            header("location: index.php?error=sqlerror1");
                            exit();
                            ...
                            $resultl = mysqli_stmt_get_result($result);
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                $sql = "UPDATE login SET login_name=?, login_email=? WHERE login_email=?";
                                $stmt = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($stmt, $sql)) {
                                    header("location: index.php?error=sqlerror");
                                    exit();
